...
bom <- try(boost_this(siX$y,data,datals, siX$last, delta, siX$id, 
                      siX$time, .1,.1,
                      m_stop_l = mseq[m[1]], m_stop_ls=mseq[m[2]],.1, betatimeind=7))
if(is.list(bo)){
  save(bom, file=paste("boost_",setup,"_",k ,".Rdata", sep=""))}
rm(bom)
rm(bo)
}
